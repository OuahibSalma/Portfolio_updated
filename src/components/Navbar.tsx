...
const Navbar = () => {
  ...
  return (
    ...
    >
      <div className="flex flex-col h-full justify-between py-4 px-2 bg-white dark:bg-gray-800 shadow-md transition-transform duration-300 w-16 lg:w-48">
          ...
          <ul className="flex flex-col space-y-3 items-start">
            {navItems.map((item) => (
              <li key={item.name} className="w-full">
                ...
                >
                  <item.icon size={isMobile ? 24 : 20} className="mx-auto lg:mx-0" />
        ...
        <div className="flex flex-col gap-1 items-start px-2 mt-2">
          ...
          >
            <Languages size={isMobile ? 24 : 20} className="mx-auto lg:mx-0" />
            <span className="hidden lg:inline-block">
              {language === 'fr' ? 'EN' : 'FR'}
            </span>
          </button>
          ...
          >
            {theme === 'dark' ? (
              <Sun size={isMobile ? 24 : 20} className="mx-auto lg:mx-0" />
            ) : (
              <Moon size={isMobile ? 24 : 20} className="mx-auto lg:mx-0" />
            )}
            <span className="hidden lg:inline-block">
